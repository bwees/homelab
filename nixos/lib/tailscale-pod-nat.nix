...
in
{
  # Lets tailscaled in a pod hole-punch like the hosts do, with no port forward.
  #
  # flannel masquerades pod egress with `--random-fully`, giving a fresh source port per
  # destination; tailscaled sees a different endpoint per DERP server, reports
  # `MappingVariesByDestIP: true` and falls back to a relay. Plain MASQUERADE keeps the
  # source port, so the pod inherits this router's endpoint-independent mapping.
  #
